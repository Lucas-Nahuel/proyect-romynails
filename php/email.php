<?php
    if(isset($_POST['enviar'])){//si el botón con el name enviar es pulsado
        if(!empty($_POST['name'])&&!empty($_POST['surname'])&&!empty($_POST['email'])&&!empty($_POST['mobile'])&&!empty($_POST['service'])){
        ...
        //guardo el contenido de cada campo en variables
            ...
            $asunto="Pedido de turno";//puedo poner un input asunto o crearlo yo directamente  
            $msg="Nombre: ".$nombre."\n"."Apellido :".$apellido."\n"."Celular :".$celular."\n".$_POST['service'];
